...
test("the snapshot runs from the hook's cwd with no --room pin", () => {
  ...
  run(
    { ...BASE, hook_event_name: "beforeSubmitPrompt", session_id: "s-cwd" },
    { stateDir: freshStateDir() }
  );
  assert.deepEqual(
...
test("already-surfaced events dedupe to {} and new mail surfaces alone mid-turn", () => {
  ...
  const repeat = run(
    { ...BASE, hook_event_name: "beforeSubmitPrompt", session_id: "s-dedupe" },
    { stateDir }
  );
  ...
  assert.equal(midTurn.additional_context, midTurn.hookSpecificOutput.additionalContext);
...
test("PostToolUse is throttled by state-file mtime", () => {
  ...
  const prompt = run(
    { ...BASE, hook_event_name: "beforeSubmitPrompt", session_id: "s-throttle" },
    { stateDir, throttleMs: 60000 }
  );
  assert.deepEqual(prompt, {});
  assert.equal(stubCalls().length, before + 1, "beforeSubmitPrompt always scans");
});
...
test("a failing post emits one diagnostic per streak, never a fake empty", () => {
  ...
  setStub({ exit: 1 });
  const first = run(
    { ...BASE, hook_event_name: "beforeSubmitPrompt", session_id: "s-fail" },
    { stateDir }
  );
...
test("malformed or unknown nonempty snapshot output fails closed", () => {
  ...
  for (const [name, stdout] of [
    ...
    setStub({ stdout });
    const out = run(
      { ...BASE, hook_event_name: "beforeSubmitPrompt", session_id: `s-${name}` },
      { stateDir: freshStateDir() }
    );
...
test("a huge distinct-channel backlog bounds the channel summary", () => {
  ...
  setStub({ events: channels });
  assert.deepEqual(
    run({ ...BASE, hook_event_name: "beforeSubmitPrompt", session_id: "s-huge-channel" }, { stateDir }),
    {},
...
test("an over-cap backlog is delivered once, then identical snapshots stay silent", () => {
  ...
  setStub({ events: mail });
  assert.deepEqual(
    run({ ...BASE, hook_event_name: "beforeSubmitPrompt", session_id: "s-overcap" }, { stateDir }),
    {},
...
test("a new arrival after an over-cap backlog still notifies with only the new id", () => {
  ...
  const out = run(
    { ...BASE, hook_event_name: "beforeSubmitPrompt", session_id: "s-overcap-arrival" },
    { stateDir }
  );
  const context = out.hookSpecificOutput.additionalContext;
...
test("consumed ids drop from state while every still-unread id is kept", () => {
  ...
  assert.deepEqual(
    run(
      { ...BASE, hook_event_name: "beforeSubmitPrompt", session_id: "s-overcap-consume" },
      { stateDir }
    ),
    {},
...
test("a closed stdout leaves fresh events and failure eligibility intact", async () => {
    ...
    await new Promise((resolve, reject) => {
      const child = spawn(process.execPath, [ADAPTER], {
        env: {
          ...
          POST_CURSOR_HOOK_BIN: STUB,
          POST_CURSOR_HOOK_STATE_DIR: stateDir,
          POST_CURSOR_HOOK_THROTTLE_MS: "0",
          STUB_CONTROL: CONTROL,
          STUB_CALLS: CALLS,
        },
  ...
  const stillFresh = run(
    { ...BASE, hook_event_name: "beforeSubmitPrompt", session_id: sessionId },
    { stateDir }
  );
  assert.match(
