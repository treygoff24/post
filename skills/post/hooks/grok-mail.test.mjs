...
test("channel-only snapshot names the channels, not a phantom mail room", () => {
  ...
  const out = run(
    { ...BASE, hookEventName: "UserPromptSubmit", session_id: "s-chanonly" },
    { stateDir: freshStateDir() }
  );
...
test("the snapshot runs from the hook's cwd with no --room pin", () => {
  ...
  run(
    { ...BASE, hookEventName: "UserPromptSubmit", session_id: "s-cwd" },
    { stateDir: freshStateDir() }
  );
  assert.deepEqual(
...
test("missing, relative, or non-string cwd fails open without spawning post", () => {
  ...
  for (const cwd of [undefined, "relative/path", 42]) {
    const input = { hookEventName: "UserPromptSubmit", session_id: "s-nocwd" };
...
test("already-surfaced events dedupe to {} and new mail surfaces alone mid-turn", () => {
  ...
  setStub({ events: [MAIL_A] });
  run({ ...BASE, hookEventName: "UserPromptSubmit", session_id: "s-dedupe" }, { stateDir });

  const repeat = run(
    { ...BASE, hookEventName: "UserPromptSubmit", session_id: "s-dedupe" },
    { stateDir }
  );
  ...
  assert.equal(midTurn.hookSpecificOutput.hookEventName, "UserPromptSubmit");
...
test("a new session id surfaces the still-unread backlog again", () => {
  const stateDir = freshStateDir();
  setStub({ events: [MAIL_A] });
  run({ ...BASE, hookEventName: "UserPromptSubmit", session_id: "s-reset" }, { stateDir });
  const again = run(
    { ...BASE, hookEventName: "UserPromptSubmit", sessionId: "s-reset-other" },
    { stateDir }
  );
...
test("user_prompt_submit, hook_event_name, and GROK_HOOK_EVENT all scan", () => {
  setStub({ events: [MAIL_A] });
  const snake = run(
...
test("malformed or unknown nonempty snapshot output fails closed", () => {
  ...
  for (const [name, stdout] of [
    ...
    setStub({ stdout });
    const out = run(
      { ...BASE, hookEventName: "UserPromptSubmit", session_id: `s-${name}` },
      { stateDir: freshStateDir() }
    );
...
test("missing or empty session_id fails open without spawning post", () => {
  ...
  for (const session_id of [undefined, "", "   "]) {
    const input = { hookEventName: "UserPromptSubmit", cwd: CWD };
...
test("unreadable ids and channel metadata are count-only", () => {
  ...
  const out = run(
    { ...BASE, hookEventName: "UserPromptSubmit", session_id: "s-escape" },
    { stateDir: freshStateDir() }
  );
  const context = out.hookSpecificOutput.additionalContext;
...
test("valid unreadable events stay count-only and never echo the id", () => {
  ...
  const out = run(
    { ...BASE, hookEventName: "UserPromptSubmit", session_id: "s-unreadable-ok" },
    { stateDir: freshStateDir() }
  );
  const context = out.hookSpecificOutput.additionalContext;
...
test("state write refuses a planted predictable legacy temp symlink", () => {
  ...
  setStub({ events: [MAIL_A] });
  const out = run(
    { ...BASE, hookEventName: "UserPromptSubmit", session_id: sessionId },
    { stateDir }
  );
  assert.match(out.hookSpecificOutput.additionalContext, /20260730-010101-aaa111/);
...
test("SessionStart does not prune arbitrary sibling state", () => {
  ...
  setStub({ events: [] });
  run({ ...BASE, hookEventName: "UserPromptSubmit", session_id: "s-prune" }, { stateDir });
...
test("a huge distinct-channel backlog bounds the channel summary", () => {
  ...
  const out = run(
    { ...BASE, hookEventName: "UserPromptSubmit", session_id: "s-huge-channel" },
    { stateDir }
  );
  const context = out.hookSpecificOutput.additionalContext;
  ...
  setStub({ events: channels });
  assert.deepEqual(
    run({ ...BASE, hookEventName: "UserPromptSubmit", session_id: "s-huge-channel" }, { stateDir }),
    {},
...
test("an over-cap backlog is delivered once, then identical snapshots stay silent", () => {
  ...
  setStub({ events: mail });
  const first = run(
    { ...BASE, hookEventName: "UserPromptSubmit", session_id: "s-overcap" },
    { stateDir }
  );
  ...
  setStub({ events: mail });
  assert.deepEqual(
    run({ ...BASE, hookEventName: "UserPromptSubmit", session_id: "s-overcap" }, { stateDir }),
    {},
...
test("a new arrival after an over-cap backlog still notifies with only the new id", () => {
  ...
  const mail = overCapMail();
  setStub({ events: mail });
  run({ ...BASE, hookEventName: "UserPromptSubmit", session_id: "s-overcap-arrival" }, { stateDir });

  const newcomer = { ...MAIL_A, id: "20260722-040404-ddd444" };
  setStub({ events: [...mail, newcomer] });
  const out = run(
    { ...BASE, hookEventName: "UserPromptSubmit", session_id: "s-overcap-arrival" },
    { stateDir }
  );
  const context = out.hookSpecificOutput.additionalContext;
...
test("consumed ids drop from state while every still-unread id is kept", () => {
  ...
  const mail = overCapMail();
  setStub({ events: mail });
  run({ ...BASE, hookEventName: "UserPromptSubmit", session_id: "s-overcap-consume" }, { stateDir });

  const remaining = mail.slice(0, 1500);
  setStub({ events: remaining });
  assert.deepEqual(
    run(
      { ...BASE, hookEventName: "UserPromptSubmit", session_id: "s-overcap-consume" },
      { stateDir }
    ),
    {},
